...
func (r Raw) MarshalJSON() ([]byte, error) { // nolint: unparam
	return r, nil
}

func (r *Raw) UnmarshalJSON(data []byte) error { // nolint: unparam
	*r = data
	return nil
}
...
func (api *APIClient) Do(request Request) (*Response, *Error) {
	...
	for attempt := 1; attempt < 5; attempt++ {
		res, err = api.httpClient.Do(req) // nolint: bodyclose
		if err == nil {
			break
		}
	...
	response, vkErr := Process(res.Body)
	if vkErr == nil {
...
func (r Request) HTTPGet() (req *http.Request) {
	u := APIURL()
...
func (r Request) HTTPPost() (req *http.Request) {
	u := APIURL()
	u.Path = path.Join(u.Path, r.Method)

	req, err := http.NewRequest(http.MethodPost, u.String(), strings.NewReader(r.Values.Encode()))
	if err != nil {
		panic(err)
	}
